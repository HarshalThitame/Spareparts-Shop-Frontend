...
@Component({
  selector: 'app-admin-unpaid-orders',
  templateUrl: './admin-unpaid-orders.component.html',
  styleUrl: './admin-unpaid-orders.component.css'
})
export class AdminUnpaidOrdersComponent implements OnInit {
  orders: Order[] = [];
  ...
  ngOnInit(): void {
    this.loadUnpaidOrders(); // Load initial data
  }

  loadUnpaidOrders(): void {
    this._adminOrderService.getUnpaidOrders(this.pageIndex, this.pageSize).subscribe(data => {
  ...
  showNoOrdersAlert(): void {
    Swal.fire({
      icon: 'info',
      title: 'No Unpaid Orders',
      text: 'There are currently no unpaid orders available.',
  ...
  onPageChange(event: PageEvent): void {
    ...
    this.loadUnpaidOrders(); // Load data for new page
  }
}
